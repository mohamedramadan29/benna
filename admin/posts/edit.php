...
if (isset($_POST['edit_cat'])) {
    ...
    if (empty($formerror)) {
        $stmt = $connect->prepare("UPDATE posts SET cat_id=?,name=?,short_desc=?,description=?,date=? WHERE id = ? ");
        $stmt->execute(array($cat_id,$name,$short_desc, $description,$date, $post_id));
        if (!empty($_FILES['main_image']['name'])) {
